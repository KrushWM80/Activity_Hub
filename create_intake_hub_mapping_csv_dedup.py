...
# Query DISTINCT Intake Hub projects (deduplicated) - ACTIVE ONLY
sql = """
SELECT DISTINCT
    PROJECT_ID,
    Title,
...
print("Fetching distinct projects from Intake Hub...")
...
print(f"Total DISTINCT projects found: {len(projects_data)}")
...
for area in sorted(summary.keys()):
    print(f"  {area:<40} {summary[area]:>8} projects")

print(f"\nTotal: {len(projects_data)} unique projects")
print(f"\nBusiness Area Mapping Applied:")
print("-" * 80)
for current, updated in sorted(BUSINESS_AREA_MAPPING.items()):
    print(f"  {current:<45} → {updated}")
