"""Direct cache population with corrected Realty project_id - WITH BETTER ERROR HANDLING"""
import sys
import os
import sqlite3
...
try:
    ...
    for row in result:
        ...
        if len(batch) >= 10000:
            ...
            conn.commit()
            print(f"  Inserted {row_count:,} rows (batch committed)...")
            batch = []
    ...
    # Final commit
    conn.commit()
    print(f"\n✅ All {row_count:,} rows committed to database")
    ...
    conn.close()
    print(f"\n🎉 Cache ready! Restart server to load updated data.")
