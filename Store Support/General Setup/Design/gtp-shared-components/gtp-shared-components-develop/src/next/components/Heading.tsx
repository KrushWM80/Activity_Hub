...
export type HeadingWeight = typeof weight400 | typeof weight700;

export type HeadingProps = CommonTextProps & {
  /**
   * The text of the Heading.
   */
  children: React.ReactNode;
  /**
   * The size for the Heading.
 ...
 * ```
 */
const Heading = React.forwardRef<Text, HeadingProps>((props, ref) => {
  const {
    children,
    size = 'medium',
    weight = weight700,
    color = 'gray160', // token.componentTextHeadingTextColor
    UNSAFE_style = {},
    ...rest
  } = props;
  ...
  return (
    <Text
      ...
      testID={Heading.displayName}
      ref={ref}
      style={[ss(weight as Weights, size, localColor).text, UNSAFE_style]}
      {...rest}>
      {children}
    </Text>
  );
...
const _getFontSize = (size: HeadingSize) => {
  if (size === 'small') {
    return token.componentTextHeadingSizeSmallFontSizeBS;
  } else if (size === 'large') {
    return token.componentTextHeadingSizeLargeFontSizeBS;
  }
  return token.componentTextHeadingSizeMediumFontSizeBS;
};

//exporting testing purpose
const _getLineHeight = (size: HeadingSize) => {
  if (size === 'small') {
    return token.componentTextHeadingSizeSmallLineHeightBS;
  } else if (size === 'large') {
    return token.componentTextHeadingSizeLargeLineHeightBS;
  }
  return token.componentTextHeadingSizeMediumLineHeightBS;
};
// ---------------
// Styles
// ---------------
const ss = (weight: Weights, size: HeadingSize, colors: TypographyColors) => {
