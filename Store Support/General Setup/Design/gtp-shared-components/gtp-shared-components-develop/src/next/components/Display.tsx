...
export type DisplayProps = CommonTextProps & {
  /**
   * The text of the Display.
   */
  children: React.ReactNode;
  /**
   * The size for the Display.
 ...
 * ```
 */
const Display = React.forwardRef<Text, DisplayProps>((props, ref) => {
  const {
    children,
    size = 'large',
    weight = weight400,
    color = 'gray160', // token.componentTextDisplayTextColor
    UNSAFE_style = {},
    ...rest
  } = props;
  ...
  return (
    <Text
      ...
      testID={Display.displayName}
      ref={ref}
      style={[ss(localWeight as Weights, size, localColor).text, UNSAFE_style]}
      {...rest}>
      {children}
    </Text>
  );
...
// Styles
// ---------------
const ss = (weight: Weights, size: DisplaySize, colors: TypographyColors) => {
  // values from legacy Display
  // const fontSize = size === 'small' ? 18 : 24;
  // const lineHeight = size === 'small' ? 24 : 32;

  // values from LD3 specs - they look disproportionally big for mobile!
  // see convo with Design team: https://walmart.slack.com/archives/GTNL5A4LS/p1657552745748649
  const fontSize =
    size === 'small'
      ? token.componentTextDisplaySizeSmallFontSizeBS // 28
      : token.componentTextDisplaySizeLargeFontSizeBS; // 32
  const lineHeight =
    size === 'small'
      ? token.componentTextDisplaySizeSmallLineHeightBS // 36
      : token.componentTextDisplaySizeLargeLineHeightBS; // 40
