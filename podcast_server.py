...
METADATA_DIR = Path("Store Support/Projects/AMP/Zorro/output/metadata")
...
class PodcastHandler(SimpleHTTPRequestHandler):
    ...
    def get_podcasts_list(self):
        ...
        if PODCASTS_DIR.exists():
            for file in sorted(PODCASTS_DIR.glob("*.wav"), key=os.path.getmtime, reverse=True):
                podcasts.append({
                    "filename": file.name,
...
<body>
    <div class="container">
        ...
        <h1>Your Week 4 Messages are Here</h1>
        <div class="product-type">🔊 Audio Format</div>
        <div class="subtitle">Multi-voice narration of store operations</div>
        ...
        async function loadAudioFiles() {
            try {
                ...
                container.innerHTML = podcasts.map(podcast => {
                    ...
                    const voiceName = podcast.filename.split(" - ").pop().replace(".wav", "");
                    const voiceBadge = `<span class="voice-badge">${voiceName}</span>`;
                    
                    return `
                    <div class="podcast-item">
                        <div class="podcast-title">
                            Audio Narration ${voiceBadge}
                        </div>
                        <div class="podcast-info">
                            <span>Size: ${podcast.size_mb} MB</span>
                            <span>Format: WAV (16-bit)</span>
                        </div>
                        <audio controls>
...
if __name__ == "__main__":
    os.chdir("C:\\Users\\krush\\Documents\\VSCode\\Activity-Hub")
    server_address = ("", 8888)
